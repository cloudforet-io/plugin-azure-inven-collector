...
class VirtualMachineVmManager:
    ...
    def get_vm_info(self, vm, disks, nics, resource_group, subscription, network_security_groups, primary_ip,
                    skus_dict):
        '''
        server_data = {
            "name": ""
    ...
    def get_azure_data(self, vm):
        ...
        if getattr(vm, 'diagnostics_profile') and getattr(vm.diagnostics_profile,
                                                          "boot_diagnostics") and vm.diagnostics_profile.boot_diagnostics:
            azure_data.update({
