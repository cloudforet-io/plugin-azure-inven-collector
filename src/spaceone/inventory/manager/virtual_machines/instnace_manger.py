...
class VirtualMachinesManager(AzureManager):
    ...
    def collect_cloud_service(self, params):
        ...
        # vms = list(azure_vm_connector.list_vms_in_rg(resource_group_name='vm-resource-group-1'))
        ...
        for vm in vms:
            try:
                import copy
                # vm_temp = copy.deepcopy(vm)
                # vm_dict = self.convert_nested_dictionary(self, vm_temp)
                vnet_data = None
                subnet_data = None
                lb_vos = []
                resource_group, resource_group_name = self.get_resource_info_in_vm(vm, resource_groups)
                print(f'======== {resource_group_name}')
                disks_rg = self.get_resources_in_resource_group(disks, resource_group_name)
                print('========')
                ...
                # print('---------')
                # import pprint
                # pprint.pprint(f"virtual machine region {vm_resource}")
                # print('---------')
    ...
    def collect_cloud_service_backup(self, params):
        '''
            Args:
                params (dict):
                    - 'options' : 'dict'
        ...
        azure_vm_connector: VirtualMachineConnector = self.locator.get_connector(self.connector_name, **params)
        azure_vm_connector.set_connect(params['secret_data'])
        ...
        network_security_groups = list(azure_vm_connector.list_network_security_groups())
        network_interfaces = list(azure_vm_connector.list_network_interfaces())
        list_disks = list(azure_vm_connector.list_disks())
        public_ip_addresses = list(azure_vm_connector.list_public_ip_addresses())
        virtual_networks = list(azure_vm_connector.list_virtual_networks())
