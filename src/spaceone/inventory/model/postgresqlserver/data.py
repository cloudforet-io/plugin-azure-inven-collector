...
class ResourceIdentity(Model):
    ...
    type = StringType(choices=('SystemAssigned', ''), serialize_when_none=False)
...
class Server(Model):
    ...
    minimal_tls_version = StringType(serialize_when_none=False)
    ...
    replicas = ListType(ModelType(ReplicaServer), serialize_when_none=False)
    server_administrators = ListType(ModelType(ServerAdministratorResource), serialize_when_none=False)
