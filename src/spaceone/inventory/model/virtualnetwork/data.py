...
class VirtualNetworkBgpCommunities(Model):
    regional_community = StringType(serialize_when_none=False)
    virtual_network_community = StringType(serialize_when_none=False)
...
class VirtualNetworkPeering(Model):
    ...
    etag = StringType(serialize_when_none=False)
    name = StringType()
    allow_forwarded_traffic = BooleanType(serialize_when_none=False)
...
class VirtualNetwork(Model):  # Main Class
    ...
    extended_location = ModelType(ExtendedLocation, serialize_when_none=False)
    address_space = ModelType(AddressSpace, serialize_when_none=False)
    bgp_communities = ModelType(VirtualNetworkBgpCommunities, serialize_when_none=False)
    ddos_protection_plan = ModelType(SubResource, serialize_when_none=False)
    dhcp_options = ModelType(DhcpOptions, serialize_when_none=False)
    enable_ddos_protection = BooleanType(default=False, serialize_when_none=False)
    enable_vm_protection = BooleanType(serialize_when_none=False)
    ip_allocations = ListType(ModelType(SubResource), serialize_when_none=False)
    provisioning_state = StringType(choices=('Deleting', 'Failed', 'Succeeded', 'Updating'), serialize_when_none=False)
    resource_guid = StringType(serialize_when_none=False)
    service_endpoints = ListType(ModelType(ServiceEndpointPropertiesFormat), serialize_when_none=False)
