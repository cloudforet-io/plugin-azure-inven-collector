...
class ApplicationSecurityGroup(Model): # belongs to NetworkInterfaceIPConfiguration
...
class PublicIPAddress(Model):
    ...
    idle_timeout_in_minutes = IntType(serialize_when_none=False)
    ip_address = StringType(serialize_when_none=False)
    ip_configuration = ModelType(IPConfigurationRef, serialize_when_none=False)
...
class VirtualNetworkTap(Model):
    ...
    network_interface_tap_configurations = ListType(ModelType(NetworkInterfaceTapConfiguration))  # 아래에 있음
    provisioning_state = StringType(choices=('Deleting', 'Failed', 'Succeeded', 'Updating'), serialize_when_none=False)
    resource_guid = StringType(serialize_when_none=False)
    tags = ModelType(Tags, serialize_when_none=False)
    type = StringType(serialize_when_none=False)
...
class PrivateEndpoint(Model):
    ...
    provisioning_state = StringType(choices=('Deleting', 'Failed', 'Succeeded', 'Updating'), serialize_when_none=False)
    subnet = ModelType(Subnet, serialize_when_none=False)  # 아래에 있음
    tags = ModelType(Tags, serialize_when_none=False)
    type = StringType(serialize_when_none=False)
...
class NetworkInterfaceTapConfiguration(Model):
    ...
    name = StringType(serialize_when_none=False)
    provisioning_state = StringType(choices=('Deleting', 'Failed', 'Succeeded', 'Updating'), serialize_when_none=False)
    virtual_network_tap = ModelType(VirtualNetworkTap, serialize_when_none=False)
    type = StringType(serialize_when_none=False)
...
class IPConfigurationProfile(Model):
    ...
    name = StringType(serialize_when_none=False)
    provisioning_state = StringType(choices=('Deleting', 'Failed', 'Succeeded', 'Updating'), serialize_when_none=False)
    subnet = ModelType(Subnet, serialize_when_none=False)  # 아래에 있음.. 흠
    type = StringType(serialize_when_none=False)
...
class LoadBalancer(Model):
    ...
    location = StringType()
    extended_location = ModelType(ExtendedLocation, serialize_when_none=False)  # TODO: (O)
    ...
    inbound_nat_rules = ListType(ModelType(InboundNatRule), serialize_when_none=False)  # TODO: ()
